...
### Plots for LANET ----
# N <- 1000;cl <- 0.9;mod <- "best clust";I <- "constant";O <- "realistic";cascade_id <- 2
N <- 1000;cl <- 0.9;mod <- "best clust";I <- "realistic";O <- "realistic";cascade_id <- 3
file.name <- paste0("chain_N_",N,"_cl_",
                    round(cl,2),"_mode_",
                    mod,"_I_",I,
                    "_O_",O,".RData")
...
file.name <- paste0("sim_","N_",N,"_cl_",round(cl,2),"_mode_",mod,"_I_",I,"_O_",O,"_")
pdf(paste0(file.name,"cascade_",cascade_id,".pdf"),
    height = 6,width = 9)
plot_cascade(tmp_sim$cascades[[cascade_id]])
dev.off()
...
# save(res_df,full_res_df,file = "diffusion_simulations_results.RData")
load("diffusion_simulations_results.RData")
...
coef(model_reach)
# O realista e I constante mejoran el alcance.

coef(model_time)
# O realista, I realista, red en 2, best clust, y random reducen la duración de la difusión.